...
export default function LoadingSkeleton({ count = 4, type = 'card' }) {
    ...
    return (
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {Array.from({ length: count }).map((_, i) => (
          <div
            key={i}
            className="rounded-xl overflow-hidden bg-white dark:bg-surface-800 shadow-card"
          >
  ...
  if (type === 'detail') {
    return (
      <div className="max-w-4xl mx-auto p-4 space-y-6">
        <div className="aspect-square max-w-md mx-auto rounded-2xl animate-shimmer" />
  ...
  if (type === 'list') {
    return (
      <div className="space-y-3">
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} className="flex gap-4 p-4 rounded-xl bg-white dark:bg-surface-800">
